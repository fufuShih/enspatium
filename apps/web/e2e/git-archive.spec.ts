...
test('Clone downloads the whole source ZIP at the viewed revision with folders and exact file bytes', async ({ page, environment }, testInfo) => {
  ...
  await expect(page.getByRole('link', { name: 'View source', exact: true })).toHaveAttribute('href', new RegExp(`/at/${second}/file/`))
  ...
  await expect(page).toHaveURL(new RegExp(`/at/${second}/file/`))
  await page.getByRole('button', { name: 'Clone', exact: true }).click()
