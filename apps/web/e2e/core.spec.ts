...
test('register, sign in, create a Space and restore the session on reload', async ({ page }) => {
  ...
  const tabs = page.getByRole('navigation', { name: 'Space navigation', exact: true })
  await expect(tabs.getByRole('link')).toHaveText(['Code', 'Settings'])
  await expect(tabs.getByRole('link', { name: 'Code', exact: true })).toHaveAttribute('aria-current', 'page')
  await page.reload()
...
test('Space settings persist the name, visibility and default branch', async ({ page, request, environment }, testInfo) => {
  const user = await register(page, 'Settings owner')
  await signIn(page, user)
  const space = await createSpace(page, 'Settings project')
  await openSettings(page)
  const tabs = page.getByRole('navigation', { name: 'Space navigation', exact: true })
  await expect(tabs.getByRole('link', { name: 'Settings', exact: true })).toHaveAttribute('aria-current', 'page')
  ...
  await page.goto(space.url + '/branch/release')
  await expect(page.getByRole('heading', { name: 'Release content', exact: true })).toBeVisible()
  await page.screenshot({ path: testInfo.outputPath('code-tab.png'), fullPage: true })
  await openSettings(page)
  await expect(tabs.getByRole('link', { name: 'Code', exact: true })).toHaveAttribute('href', space.url + '/branch/release')
  await page.screenshot({ path: testInfo.outputPath('settings-tab.png'), fullPage: true })
  await tabs.getByRole('link', { name: 'Code', exact: true }).click()
  await expect(page).toHaveURL(environment.webOrigin + space.url + '/branch/release')
  await page.setViewportSize({ width: 390, height: 844 })
  await expect.poll(() => page.evaluate(() => document.documentElement.scrollWidth <= innerWidth)).toBe(true)
  await expect(page.getByRole('heading', { name: 'Release content', exact: true })).toBeVisible()
  await page.screenshot({ path: testInfo.outputPath('code-tab-mobile.png'), fullPage: true })
  await openSettings(page)
