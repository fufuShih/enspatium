...
export async function refreshObjectLists(client: QueryClient, account: string, slug: string) {
  const keys = [getBrowseObjectsQueryKey(account, slug), getListObjectsQueryKey(account, slug), getGetObjectStorageUsageQueryKey(account, slug), getListObjectVersionsQueryKey(account, slug), getGetObjectHeadQueryKey(account, slug)]
