...
export default function SpacePage({ settings = false }: { settings?: boolean }) {
  ...
  return (
    <PageContainer>
      ...
      <Flex align={{ base: 'flex-start', sm: 'center' }} justify="space-between" direction={{ base: 'column', sm: 'row' }} gap="20px" mt="24px" mb="32px">
        ...
        <Flex gap="8px" flexShrink="0" align="center">
          {space.canManage && <ActionButton asChild minH="40px"><PageLink to={spacePath(account, space.slug) + (settings ? '' : '/settings')}>{settings ? 'Back to Space' : 'Settings'}</PageLink></ActionButton>}
