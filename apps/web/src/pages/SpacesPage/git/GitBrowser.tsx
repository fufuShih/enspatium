...
export default function GitBrowser({ account, slug }: { account: string; slug: string }) {
  ...
  if (!info.data.branches.length && !pinned && refType === 'branch') return <Box minW="0">
  ...
  return (
    <Box as="section" aria-label="Repository browser" minW="0">
      ...
      {refType === 'tag' && tags.isPending && !pinned ? <RequestState loading title="Loading tags..." /> : refType === 'tag' && tags.isError && !pinned ? <RequestState title="Unable to load tags" message={gitErrorMessage(tags.error)} onRetry={() => { void tags.refetch() }} /> : !ready ? <RequestState title={refType === 'tag' ? (branch ? 'Tag not found' : 'No tags yet') : 'Branch not found'} message={refType === 'tag' && !branch ? 'Push a Git tag to browse a saved version.' : 'Choose an existing reference to continue.'}><ActionButton mt="16px" onClick={() => { void info.refetch(); void tags.refetch() }}>Refresh references</ActionButton></RequestState> : isHistory ? <GitHistory account={account} slug={slug} branch={branch} /> : <>
