...
export default function MediaPage({ space: { account, slug, name } }: AppPageProps) {
  ...
  return <Box as="section" aria-label="Media library" minH="100dvh" bg="#111514" color="#eef2f0" css={{ '--background': '#111514', '--foreground': '#eef2f0', '--surface': '#222a27', '--border': '#323b36', '--muted': '#a5b0aa', '--accent-ink': '#91d7b5', colorScheme: 'dark', '& :is(button, a, input):focus-visible': { outline: '2px solid #91d7b5', outlineOffset: '4px' } }} onKeyDown={event => { if (event.key === 'Escape' && selected) { event.preventDefault(); closePlayer() } }}>
    <Flex maxW="1560px" mx="auto" align="center" gap="24px" p={{ base: '20px', md: '22px 32px' }} borderBottom="1px solid #ffffff0e">
      <Flex align="center" gap="10px" flexShrink="0" minW={{ md: '164px' }}><Box w="12px" h="12px" bg="#83cfa7" borderRadius="full" boxShadow="0 0 0 5px #83cfa713" /><Text fontSize="15px" fontWeight="600" letterSpacing="-.02em">Media</Text></Flex>
    ...
    <Box maxW="1560px" mx="auto" display="grid" gridTemplateColumns={{ base: '1fr', md: '208px minmax(0, 1fr)' }}>
      <Box as="nav" aria-label="Media categories" p={{ base: '16px 20px', md: '32px 20px' }} borderRight={{ md: '1px solid #ffffff0e' }}>
      ...
      <Box minW="0" p={{ base: '8px 20px 40px', md: '32px' }}>
        ...
        {selected && <Box ref={playerRegion} id="media-player" as="section" aria-label="Now playing" tabIndex={-1} mb="32px" p={{ base: '14px', md: '20px' }} border="1px solid var(--border)" borderRadius="14px" bg="#1b211e" outline="none" maxW="960px" scrollMarginTop="20px">
          <Flex align="center" justify="space-between" mb="14px" gap="12px"><Text color="#a6e4c1" fontSize="11px" fontWeight="600" letterSpacing=".1em">{selected.kind === 'image' ? 'PHOTO VIEWER' : 'NOW PLAYING'}</Text><ActionButton onClick={closePlayer} aria-label="Close player" p="6px 10px" fontSize="12px">Close<Text as="span" aria-hidden="true">×</Text></ActionButton></Flex>
