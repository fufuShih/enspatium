...
The repository browser switches between branches and commit-based tags (lightweight, annotated or nested annotated). Branch and tag names use explicit Git namespaces, so identical names remain distinct. File/history links and ZIP downloads retain the selected version. Tags pointing to trees or blobs are excluded because they have no commit history. Browse/history/diff API refs allow up to 1,024 characters, including the explicit namespace prefix.

**References** provides name search and pages of 30 branches or tags, with the default branch marked and links to files, latest commit details and history. The typed `GET /namespaces/:namespaceSlug/spaces/:spaceSlug/git/refs` API accepts `type=branch|tag`, optional `search`, `offset` and `limit` (maximum 100). It checks repository read access and reads metadata using captured commit IDs. Dates are the tip commit's committer time, not the time of a push, ref update or tag creation; no new database tracking is implied.

**Compare** selects a Base and Target branch or tag. It compares their endpoint trees directly, not a merge-base or proposed merge. Both resolved commit IDs are saved in the URL alongside the selected names and changed file. Reloads and shared links retain those snapshots; **Refresh comparison** resolves the current names again, and **Swap** reverses the displayed changes. Comparisons use the existing permission-checked diff API and the same changed-file viewer as commit details. Binary changes are identified without a text preview. Responses above 1 MiB show a clone-to-inspect message; a single file preview displays at most 3,000 diff lines. A commit removed from all refs and later pruned may no longer be available through an old link. The local production HTTPS acceptance test also verifies tag cloning, list metadata, tagged README contents and endpoint comparison after container recreation.
...
For an upgrade, follow the [application upgrade and rehearsal workflow](UPGRADE.md): build uniquely tagged candidate images, take and verify the previous release's [database/content backup](BACKUP.md), stop traffic, and start the candidate with `up -d --no-build --pull never --wait`. The migration container runs before the backend. Restoring an older image alone is insufficient when a schema change is incompatible. The backup includes matching images and can be restored into a separate deployment without overwriting the original.

### Controlled Git maintenance

In **Site administration > Git**, search by Space or namespace, select **Maintain**, and confirm the repository. The admin-only `POST /admin/git/maintenance` accepts `{ "spaceId": "<uuid>" }` and returns 202 with a job; poll `GET /admin/git/maintenance` until `phase` is `finished` and inspect `status`. The page polls automatically and can be left and reopened. `GET /admin/git/spaces` lists Git Spaces in pages of 30, with optional `search` and `cursor`. Every endpoint rechecks the current administrator role and sends `private, no-store`.

One job runs in this backend process. Its storage lease prevents all service storage mutations throughout checking, maintenance and audit recording. A full integrity check must pass before mutation. Compaction requires exclusive ownership of the Git process pool: active Git commands cause the job to fail as busy, and new Git browse/clone/push/download requests return 503 while it owns the pool. Object reads remain available. No operation queues behind maintenance; retry busy requests after it finishes.

The fixed operation is `git gc --quiet --prune=2.weeks.ago`, without aggressive or forced GC. Repository hooks are disabled; external object stores, links, integrity findings, unsupported config and custom GC recent-object hooks require inspection first. The job verifies all refs and symbolic HEAD are unchanged and runs full Git fsck afterward. It reports measured object bytes before/after; storage can grow slightly because of indexes and retained objects. Referenced history is preserved. Old unreachable objects may be deleted; the two-week threshold is not an Object version retention setting.

Admission requires free space for the configured reserve, the current object bytes and 16 MiB overhead. This is conservative headroom, not an OS reservation or a guarantee against outside writers. Run during a quiet period with a verified backup. The scan/Git commands share a four-minute deadline, each object-size scan has a 30-second limit, and the separate database connection has five-second connection and ten-second query limits. GC process-tree termination waits for child processes and streams to close before leases are released. Graceful server shutdown waits for an admitted job; the Compose six-minute grace period covers the normal bounded work.

Start and completion events (`git.maintenance_started`, `git.maintained`) persist in database audit records; Space owners can retrieve them through the existing `GET /namespaces/:namespaceSlug/spaces/:spaceSlug/audit-events` API. A failed completion audit is explicitly reported without claiming successful compaction was rolled back. Only the current/last job stays in memory; a server restart resets that status. A start event without a completion event after an abrupt restart needs inspection. A killed or failed GC can leave temporary files/locks; use the Storage check and investigate with the service stopped before retrying. This feature neither resumes interrupted jobs nor removes reported lock files automatically. Keep one backend and do not edit `DATA_ROOT` externally while it is running.

Reference: [Git GC retention and concurrent-write behavior](https://git-scm.com/docs/git-gc/2.38.0).
...
- [x] Tag browsing with files/history and ZIP downloads. Browser verification covers same-name branches/tags, lightweight and nested annotated tags, Unicode names, pinned file/history links and ZIP contents, missing tags, tag-only repositories, private access, and desktop/mobile themes.
- [x] Branch/tag lists with commit and update details. The typed, permission-checked refs API returns searchable pages with pinned tip commit metadata. The UI links to files/history/commit details, marks the default branch, and labels dates as last commit time rather than push time. Browser coverage includes pagination, search, same-name and nested tags, non-commit tag exclusion, reload and mobile layout.
- [x] Branch/tag comparison with changed files and diff. Comparisons pin both commit IDs in the URL, support swapped direction and explicit refresh, and reuse the commit diff viewer. Browser verification covers same-name Branch/Tag choices, additions/deletions/renames/binary and Unicode files, reload, a later push, identical versions, unavailable refs, the 1 MiB limit, private access and desktop/mobile themes.
